...
async def simple_handoff_example():
    ...
    # 정형외과 전문의 에이전트, handoffs 파라미터가 없어 다른 에이전트로 전환할 수 없는 말단 에이전트
    정형외과의사 = Agent(
        name="Orthopedist",
    ...
    for msg in conversations:
        ...
        if current_agent != result.last_agent:
            print(f"<핸드오프 발생> {DISPLAY[current_agent.name]}에서 {DISPLAY[result.last_agent.name]}으로 핸드오프")
