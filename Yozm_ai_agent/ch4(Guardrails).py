...
# 인풋 가드레일 함수
@input_guardrail(name='콘텐츠 안전성 검사') # main agent의 input_guardrails 필드에 입력하는 함수, 사용자 입력이 main agent에 전달되기 전에 입력의 검증을 위해 실행되는 함수 
async def content_safety_guardrail(ctx, agent, input_data): # ctx: Runner에서 context로 넘긴 정보가 옴, ctx, agent를 사용하지 않음에도 정의해야하는 이유는 가드레일 함수의 인터페이스가 그렇게 정의되어 있기 때문
    """콘텐츠 안정성을 검사하는 가드레일"""
    ...
    return GuardrailFunctionOutput( # tripwire_triggered = True면 예외 발생(차단)
